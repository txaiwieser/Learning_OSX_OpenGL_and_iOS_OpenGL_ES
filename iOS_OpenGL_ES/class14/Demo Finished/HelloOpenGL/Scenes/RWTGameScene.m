...
- (void)updateWithDelta:(NSTimeInterval)dt {
  ...
  if (newY < 0) {
    ...
    bounced = YES;
    [RWTDirector sharedInstance].scene = [[RWTGameOverScene alloc] initWithShader:self.shader win:NO];
  }
  if (newX > 27.0) {
    newX = 27.0;
    _ballVelocityX = -_ballVelocityX;
  ...
  if (bounced) {
    [[RWTDirector sharedInstance] playPopEffect];
  }
  ...
  if (_bricks.count == 0) {
    [RWTDirector sharedInstance].scene = [[RWTGameOverScene alloc] initWithShader:self.shader win:YES];
  }
